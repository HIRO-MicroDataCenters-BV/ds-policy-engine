...
from app.core.usecases import RuleManagementUsecase
from app.rest_api.serializers import PaginationParams, RuleCreate, RuleUpdate


@pytest.fixture
def service(mock_repo_with_seed):
    return RuleManagementUsecase(repository=mock_repo_with_seed)


@pytest.fixture
def empty_service(mock_repo):
    return RuleManagementUsecase(repository=mock_repo)
...
class TestCreateRule:
    ...
    async def test_create_rule_custom_permission_succeeds(self, empty_service):
        ...
        data = RuleCreate(
            ...
            role="catalog_owner",
            permissions=["data-export:execute"],
        )
        rule = await empty_service.create_rule(data)
    ...
    async def test_create_rule_defaults_enabled(self, empty_service):
        ...
        data = RuleCreate(
            name="Test", role="catalog_consumer", permissions=["catalog:read"]
        )
        rule = await empty_service.create_rule(data)
