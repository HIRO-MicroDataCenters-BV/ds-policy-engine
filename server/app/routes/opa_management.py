...
@router.get("/policies", summary="List OPA policies", description="List all policies currently loaded in OPA")
async def list_opa_policies(
    ...
    return success_response(data={"policies": policies}, message="OPA policies retrieved")


@router.delete("/policies/{policy_id}", summary="Delete OPA policy", description="Remove a policy from OPA")
async def delete_opa_policy(
    ...
    msg = f"Policy '{policy_id}' deleted" if deleted else f"Policy '{policy_id}' not found"
