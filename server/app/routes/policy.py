...
@router.post("/evaluate", summary="Evaluate policy",
             description="Evaluate user permissions based on role and institute")
async def evaluate_policy(
    request: PolicyEvaluationRequest,
    service: PolicyEvaluationService = Depends(get_policy_evaluation_service),
):
...
@router.post("/deploy", summary="Deploy policies to Policy Agent",
             description="Generate Rego and push to the policy agent")
async def deploy_policies(
    body: DeployRequest | None = None,
    service: PolicyDeployerService = Depends(get_policy_deployer_service),
):
...
@router.post("/preview", summary="Preview generated Rego",
...
@router.get("/deploy-history", summary="Deploy history",
...
async def deploy_history(
    ...
    return success_response(data={"history": history}, message="Deploy history retrieved")


@router.get("/overview", summary="Policy manager overview",
            description="Stats, conflicts, and deploy info for the manager dashboard")
async def policy_overview(
    deployer: PolicyDeployerService = Depends(get_policy_deployer_service),
    rule_svc: RuleManagementService = Depends(get_rule_management_service),
):
    ...
    warnings = []
    if disabled_count > 0:
        disabled_names = [r.get("name", r.get("id", "?")) for r in all_rules if not r.get("enabled", True)]
        warnings.append({
            "type": "disabled_rules",
